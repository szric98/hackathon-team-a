			...
			{items.map(({ itemLabel, onClick, dropdownMenuItems }, index) => (
				...
				>
					...
					{index === 0 && icon && (
						<Icon
							icon={icon}
							className="mr-1 size-3 fill-gray-500 group-hover:fill-brand-700 dark:fill-gray-400 "
						/>
					)}
					<span
						className={textVariants({
							isLink: !!onClick,
							isDropdown: !!dropdownMenuItems?.length,
						})}
					>
						{itemLabel}
					</span>
...
const textVariants = /*tw:*/ cva<VariantSettings>(
	"ms-1 font-medium text-sm md:ms-2 dark:text-gray-400",
	{
		variants: {
			isLink: {
