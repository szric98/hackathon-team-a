...
export const COLOR_VALUES = ["default", "gray", "red", "yellow", "green"] as const;
...
export type BadgeProps = {
  ...
  action?: WithAnalyticsTag<{ onClick: React.MouseEventHandler<SVGElement>; icon?: AvailableIcons }>;
};
...
export const Badge: FC<BadgeProps> = ({
  ...
  const primaryIconClassname = primaryIconVariants({ size, position: iconPosition });
  const secondaryIconClassName = secondaryIconVariants({ color, size });

  const BadgeTypography = typographyForSize(size);

  return (
    <BadgeTypography className={twMerge(badgeClassName, className)} onClick={onClick}>
...
const badgeVariants = /*tw:*/ cva<BadgeVariant>("inline-flex items-center gap-1 rounded-md", {
  variants: {
    color: {
      default:
...
type PrimaryIconVariant = Pick<VariantSettings, "size"> & { position: Record<"left" | "right", string> };
