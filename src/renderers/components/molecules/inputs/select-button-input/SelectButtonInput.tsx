      ...
      <div className="flex flex-wrap gap-3">
        {options.map((option) => (
          <button
            key={getKey(option)}
            type="button"
            className={selectedButton({ isSelected: getKey(option) === selected })}
            onClick={() => handleClick(option)}
            disabled={disabled || (disableOption ? disableOption(option) : false)}
          >
            <Typography.Caption
              className={
                disabled || disableOption?.(option)
                  ? "text-gray-300"
                  : selectedButtonText({ isSelected: getKey(option) === selected })
              }
            >
              {getLabel(option)}
            </Typography.Caption>
