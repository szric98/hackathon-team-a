...
export const Toggle: FC<ToggleProps> = ({ checked, onCheckedChange, size = "md", label, helperText, disabled }) => {
  const rootClasses = toggleRootVariants({ size, disabled });
  const thumbClasses = toggleThumbVariants({ size });
  return (
    <div className="flex gap-3">
      <RadixToggle.Root checked={checked} onCheckedChange={onCheckedChange} disabled={disabled} className={rootClasses}>
        <RadixToggle.Thumb className={thumbClasses} />
      </RadixToggle.Root>
      <div className="flex flex-col">
        <Typography.CaptionMedium color={disabled ? "disabled" : "primary"}>{label}</Typography.CaptionMedium>
        <Typography.SubCaption color={disabled ? "disabled" : "secondary"}>{helperText}</Typography.SubCaption>
      </div>
    </div>
  );
};
