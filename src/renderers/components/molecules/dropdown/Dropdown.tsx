...
type DropdownMenuItemNested = Omit<DropdownProps, "Trigger"> & DropdownMenuItemSimple;

export type DropdownMenuItem = DropdownMenuItemSimple | DropdownMenuItemNested;

export type DropdownMenuItemWithRequiredLabel = DropdownMenuItem & { label: string };
...
const contentVariants = /*tw:*/ cva<VariantSettings>(
  "z-[1001] w-fit min-w-0 divide-y divide-gray-100 overflow-x-hidden rounded-lg bg-white py-1 text-gray-900 shadow-md transition-opacity duration-100 focus:outline-none dark:bg-gray-700 dark:text-white",
  {
    variants: {
      ...
      menuWidth: {
        auto: "",
        trigger: "min-w-[var(--radix-dropdown-menu-trigger-width)] max-w-[var(--radix-dropdown-menu-trigger-width)]",
      },
    },
  },
);
...
export const Dropdown: FC<DropdownProps> = ({
  ...
  const { applySearchFilterFn, changeSearchTerm } = useSearchFilter<DropdownMenuItem>({
    predicate: (item) => `${item.label ?? ""} ${item.category ?? ""}`.toLowerCase(),
  });

  const filteredData = applySearchFilterFn(menuItems);

  const groupedItems = filteredData.reduce<Record<string, DropdownMenuItem[]>>((acc, item) => {
  ...
  const content =
    withSearchInput || showCategories ? (
      <div>
        {withSearchInput && <SearchInput onChange={changeSearchTerm} className="mb-4 px-4 pt-4" />}
        <div className="flex max-h-44 flex-col gap-2 overflow-auto">
          {Object.entries(groupedItems).map(([category, items]) => (
            <div key={category} className="flex flex-col gap-4">
              {category !== UNCATEGORIZED_KEY && (
                <Typography.CaptionBold className="px-4">{category}</Typography.CaptionBold>
              )}
              <div>
                <DropdownMenuItems menuItems={items} withSearchInput={withSearchInput} onSelect={handleSelect} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <span data-analytics-id={dataAnalyticsId} className="flex" onClick={(e) => e.stopPropagation()}>
      <DropdownMenu.Root
        open={isOpen}
        onOpenChange={(open) => {
          if (disabled) return;
      ...
      >
        <DropdownMenu.Trigger aria-label={ariaLabel} className="group w-full focus:outline-none">
          {Trigger}
        </DropdownMenu.Trigger>
        <DropdownMenu.Portal>
          <DropdownMenu.Content
            sideOffset={8}
            className={twMerge(contentVariants({ withSearchInput: !!withSearchInput, menuWidth }), className)}
          >
            {content}
          </DropdownMenu.Content>
        </DropdownMenu.Portal>
...
type DropdownContentProps = Pick<DropdownProps, "menuItems"> & { onSelect?: VoidFunction; withSearchInput?: boolean };

const DropdownMenuItems: FC<DropdownContentProps> = ({ menuItems, withSearchInput = false, onSelect }) => {
  return menuItems.map((menuItem) => {
    const { key, label, onClick, dataAnalyticsId, icon, iconClassName, iconElement, disabled } = menuItem;
    ...
    return (
      <DropdownMenu.Sub key={label}>
        <DropdownMenuItem
          ...
          nested
        />
        <DropdownMenu.SubContent sideOffset={8} className={contentVariants({ withSearchInput: !!withSearchInput })}>
          {menuItem.menuItems.map((subMenuItem) => {
            const { key, label, icon, iconClassName, iconElement, onClick, dataAnalyticsId } = subMenuItem;
  ...
  const content = (
    ...
    >
      {iconElement ??
        (icon &&
          IconRenderFn(icon)({
            className: twMerge(/*tw:*/ "fill-gray-500 size-3.5 shrink-0", iconClassName),
          }))}
      <Typography.Caption className="truncate text-gray-700 dark:text-gray-300">{label}</Typography.Caption>
    </button>
  );
