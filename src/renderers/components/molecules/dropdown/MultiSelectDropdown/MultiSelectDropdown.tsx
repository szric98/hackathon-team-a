...
export type MultiSelectDropdownProps = WithAriaLabel<{
	...
	onCloseDropdown?: (
		selectedItemKeySet: KeySet<string>,
		mode: SELECTION_MODE,
	) => void;
	label?: string;
}>;
...
export const MultiSelectDropdown: FC<MultiSelectDropdownProps> = ({
	...
	const [selectedItems, setSelectedItems] = useState<Map<string, BaseKeyLabel>>(
		defaultSelectedList ?? new Map(),
	);
	...
	const handleDropdownClose = () => {
		if (isEmpty(selectedItems))
			return onCloseDropdown?.(new KeySetNone<string>(), selectionMode);
		if (selectedItems.size === items.size)
			return onCloseDropdown?.(new KeySetAll<string>(), selectionMode);
		return onCloseDropdown?.(
			new KeySetSome<string>(
				Array.from(selectedItems.values()).map((x) => x.key),
			),
			selectionMode,
		);
	};

	const handleSelectionModeChange = (newMode: SELECTION_MODE) =>
	...
	return (
		<DropdownMenu.Root>
			...
			>
				{label && (
					<Typography.Heading3 className="mb-4 px-4">
						{label}
					</Typography.Heading3>
				)}

				{items.size > 5 && (
					<SearchInput onChange={changeSearchTerm} className="mb-3 px-4" />
				)}
	...
	"cursor-pointer rounded-lg border px-2 py-1 font-medium text-sm outline-none",
	{
		variants: {
			active: {
				...
				false:
					"bg-gray-50 text-gray-800 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400",
			},
		},
	},
);
